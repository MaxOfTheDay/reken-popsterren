...
| C12 | `runTravel` walk (`:13520`) | level-up inside a world | two layers on one clock: the stop layer follows the road exactly (`heroRoadFrames`, sampled off the same path with `getPointAtLength`) and the gold grows from the same length; the artwork underneath (`heroWalkFrames` on `.pas-laag`) does a three-step cadence, ~4 px up and 1.5% stretch, neutral at both ends; sparkle every 280 ms; haptic pattern | `REIS_DUUR` 1400 ms after `REIS_WACHT` 400 ms | yes (map locked) | no | once | ✅ |
