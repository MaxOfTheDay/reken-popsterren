...
| C10 | `.net-af` star landing (`:3961`) | first map draw after a show, **except a level-up that travels on within the world** (see `reisBinnenWereld` in `goMap`) | star tab drops in, three stars land | 100/185/270 ms delay, 280 ms each | no | no | once | ✅ |
| C11 | `starRevealBeat` (`:12081`) | before a **world change** only — the ordinary journey no longer waits for it | *nothing* — a deliberate 550 ms + 200 ms pause | 750 ms | yes (gates the camera climb) | no | once | ✅ |
| C12 | `runTravel` walk (`:13520`) | level-up inside a world | two layers on one clock: the stop layer follows the road exactly (`heroRoadFrames`, sampled off the same path with `getPointAtLength`) and the gold grows from the same length; the artwork underneath (`heroWalkFrames` on `.pas-laag`) does a three-step cadence, ~4 px up and 1.5% stretch, neutral at both ends; sparkle every 280 ms, starting **with** the movement and without ⭐ (`REIS_VONKEN`); no departure haptic — the only travel buzz is C13's arrival tick | `REIS_DUUR` 1400 ms after `REIS_WACHT` 400 ms, which is now the map's own entry settling (`MOTION.komNa + MOTION.kom` = 300 ms) rather than a beat — movement begins ~456 ms after the map is drawn, was ~1202 ms | yes (map locked) | no | once | ✅ |
| C13 | `runTravel` arrival (`:12755`) | end of C12 | next stop dot pops, coin (whose `buzz(15)` **is** the arrival tick — there is no second one), `confetti(14)`, praise | 450 ms | no | no | once | partial |
