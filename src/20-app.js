...
/* ---- De klankjes, op de naam van wat er gebeurde ----
   ...
   Veertien regels en dertien gebeurtenissen -- 'tap' en 'tap.blij' zijn twee
   kleuren van één tik en geen twee dingen. Daarmee is de woordenschat vol. Dit is een spel voor
   vijfjarigen op één tablet in een woonkamer, geen instrument: wat er nog stil is
   -- een schermwissel, een vraag die verschijnt, elk deeltjesfeestje, de
   zaalgloed bij een goed antwoord -- is stil gehouden en niet vergeten. Zie
   docs/AUDIO-REVIEW.md §4.8 voor de plekken waar een klankje láwaai zou zijn. */
...
const SFX = {
  ...
  /* Iets aandoen. Dit stond hier als 'reward.claim' op de lijst, maar de drie
     plekken die overbleven doen alle drie hetzelfde en het is geen beloning: je
     zet iets op. Het cadeau-moment zelf -- een trofee die openknalt -- heeft zijn
     eigen fanfare, dus wat hier nog te benoemen viel is de handeling.

     Hij ligt láger dan de twee tikken erboven, en dat is het hele verschil: een
     tik zegt "ik heb je gehoord" en hoort licht te zijn, dit zegt "het zit" en
     hoort te zakken in plaats van te kletteren. Twee warme noten omhoog, en dan
     is het klaar -- de pop die iets anders aanheeft is het echte antwoord. */
  'equip':          { noten: [[587, 0.09, 0, 'triangle', 0.11], [740, 0.20, 0.07, 'triangle', 0.12]], tril: 20 },
  ...
  /* En vertrekken. De wandeling over de weg duurt 1400ms en was tot nu toe de
     langste beweging van de hele voortgangslus zonder één noot: ze zette af,
     liep, en pas bij de volgende halte klonk er iets.

     Hij ligt met opzet een kwart onder de aankomst -- vertrek eindigt op D5, de
     aankomst op A5 -- zodat de twee één beweging vormen: laag weg, hoog aan. En
     hij is korter en zachter, want een vertrek is een aanzet en geen gebeurtenis.

     GEEN tril, en dat is geen vergetelheid. De enige trilling van de reis is de
     tik van aankomen; zie het lange commentaar in runTravel waar een tweede
     trilling ooit weggehaald is. */
  'travel.depart':  { noten: [[440, 0.09, 0, 'triangle', 0.09], [587, 0.14, 0.07, 'triangle', 0.10]] },
  /* Een wereld die opengaat: de onthulling van een nieuwe wereld, en de camera
     die over de grens heen klimt. Allebei leenden ze het muntje, en daarna de
     landing van 'travel.arrive' -- maar aankomen op de volgende halte en een
     héle wereld die opengaat zijn niet hetzelfde ding.

     Drie noten die opengaan in plaats van oplopen: een kwint en dan een kwart,
     met de bovenste blijvend. Bewust in D en niet in C, want in C is dit de
     eerste helft van 'celebrate.major' en dan zou een wereld die opengaat
     klinken als het begin van een wereld die uit is. */
  'world.unlock':   { noten: [[587, 0.14, 0, 'triangle', 0.14], [880, 0.14, 0.11, 'triangle', 0.14],
                              [1175, 0.34, 0.22, 'triangle', 0.12]], tril: [0, 20, 40, 30] },
...
function sndVertrek() { playSfx('travel.depart'); }
function sndWereld() { playSfx('world.unlock'); }
function sndAan() { playSfx('equip'); }
...
/* ================= Het spelogo komt binnen =================
   Eén keer per start, en alleen hier: het merkteken (de R met de ster) ploft op
   de plek van de R in het spelogo, draait om als een munt en ís dan de R van het
   woord; de ster springt eraf, landt op de ster boven "sterren", en het woord
   veegt achter hem aan open. Ruim anderhalve seconde. Het promofilmpje
   (promo/promo.html) doet hetzelfde in het groot.

   WAAROM EEN MUNT EN GEEN OVERVLOEIER. De twee R's zijn verschillend getekend --
   die van het merkteken heeft een langere staart -- en overvloeien liet dat zien
   als een dubbel beeld. Op zijn kant is een R onzichtbaar, en precies dan wordt
   er gewisseld.
   ...
   WAT HET NIET MAG:
     ...
     - wachten op het netwerk. mark.webp is een tekening onder assets/ en komt
       dus na de eerste keer uit de voorraad (zie sw.js). Is hij er niet op tijd,
       dan valt de intro weg en verschijnt het logo gewoon.

   De maten zijn in pixels van wordmark.webp (1080x360) en mark.webp (512x512),
   opgemeten op de tekeningen. Komt er een nieuwe tekening, dan kloppen ze niet
   meer: dan landt de R of de ster ernaast. De meting staat ook in promo.html. */
const LOGO_INTRO = {
  rMark: [90, 42], rWoord: [97, 62],        // linkerbovenhoek van de gele R
  rMaat: [[270, 348], [137, 206]],          // breedte tot de boog, en hoogte: mark, woord
  sterMark: [376, 289, 77],                 // middelpunt en straal tot de punt
  sterWoord: [616, 103, 50],
  rRand: 258,                               // tot hier is het woord alleen de R
  boog: .17,                                // hoe hoog de ster springt, in logobreedtes
  // de tijdlijn, in ms
  plof: 450, draai: 560, om: 130, terug: 300, sprong: 700, open: 450, klaar: 1800,
  wachtOpBeeld: 800,                        // zo lang mogen de tekeningen erover doen
};
...
function logoIntro() {
  ...
  const mark = new Image(), ster = new Image();
  mark.className = 'li-mark'; ster.className = 'li-ster';
  mark.alt = ster.alt = '';
  mark.src = ster.src = 'assets/branding/mark.webp';
  laag.appendChild(mark); laag.appendChild(ster);
  ...
  function stop() {
    ...
    woord.style.transform = woord.style.transformOrigin = '';
    woord.style.webkitMaskImage = woord.style.maskImage = '';
  ...
  Promise.all([binnen(woord), binnen(mark)]).then(() => {
    if (klaar) return;
    clearTimeout(wacht);
    if (!logoIntroMag() || !woord.offsetWidth) return stop();
    kop.appendChild(laag);
    logoIntroSpeel(woord, laag, mark, ster, stop, f => { raf = f; });
  }, stop);
}
function logoIntroStop() { if (logoIntroLoopt) logoIntroLoopt.stop(); }

function logoIntroSpeel(woord, laag, mark, ster, stop, zetRaf) {
  const L = LOGO_INTRO;
  // één bronpixel -> scherm. mark.webp krijgt dezelfde schaal, en daarna scale(sx, sy)
  // zodat zijn R precies zo groot is als die van het woord
  const W = woord.offsetWidth, k = W / 1080;
  const sx = L.rMaat[1][0] / L.rMaat[0][0], sy = L.rMaat[1][1] / L.rMaat[0][1];
  const persp = Math.round(W * .76);
  // de laag ligt precies over het logo
  ...
  // het merkteken, met zijn linkerbovenhoek van de R op die van het woord
  const A = [L.rMark[0] * k, L.rMark[1] * k];
  mark.style.width = ster.style.width = 512 * k + 'px';
  mark.style.left = (L.rWoord[0] * k - A[0]) + 'px';
  mark.style.top = (L.rWoord[1] * k - A[1]) + 'px';
  mark.style.transformOrigin = A[0] + 'px ' + A[1] + 'px';
  const c = [sx * L.rMaat[0][0] / 2 * k, sy * L.rMaat[0][1] / 2 * k];   // van hoek naar midden van de R
  const rMidden = [L.rWoord[0] * k + c[0], L.rWoord[1] * k + c[1]];
  // de ster: zijn middelpunt op (0,0), en dan met translate op zijn plek
  ster.style.left = -L.sterMark[0] * k + 'px'; ster.style.top = -L.sterMark[1] * k + 'px';
  ster.style.transformOrigin = L.sterMark[0] * k + 'px ' + L.sterMark[1] * k + 'px';
  const ster0 = [L.rWoord[0] * k + sx * (L.sterMark[0] - L.rMark[0]) * k,
                 L.rWoord[1] * k + sy * (L.sterMark[1] - L.rMark[1]) * k];
  const ster1 = [L.sterWoord[0] * k, L.sterWoord[1] * k];
  const sterDoel = L.sterWoord[2] * k / (L.sterMark[2] * k);
  // het woord draait om het midden van zijn R
  woord.style.transformOrigin = (L.rWoord[0] + L.rMaat[1][0] / 2) * k + 'px ' + (L.rWoord[1] + L.rMaat[1][1] / 2) * k + 'px';

  const tussen = (t, a, b) => Math.min(1, Math.max(0, (t - a) / (b - a)));
  const mix = (a, b, p) => a + (b - a) * p;
  const inUit = p => p < .5 ? 4 * p * p * p : 1 - Math.pow(-2 * p + 2, 3) / 2;
  const uitTerug = (p, s) => 1 + (s + 1) * Math.pow(p - 1, 3) + s * Math.pow(p - 1, 2);
  const veer = p => p === 0 || p === 1 ? p : Math.pow(2, -10 * p) * Math.sin((p * 10 - .75) * (2 * Math.PI) / 3) + 1;
  const land = L.draai + 30 + L.sprong;
  let geland = false;
  ...
  function beeld(nu) {
    ...
    // 1. het merkteken ploft erin, verend, om het midden van zijn R
    const plof = veer(tussen(t, 0, L.plof));
    // 2. de munt: tot 90 graden is het het merkteken, daarna het woord
    const om = tussen(t, L.draai, L.draai + L.om);
    mark.style.opacity = om < 1 ? 1 : 0;
    mark.style.transform = `translate(${c[0]}px, ${c[1]}px) perspective(${persp}px) rotateY(${om * om * 90}deg) scale(${plof}) translate(${-c[0]}px, ${-c[1]}px) scale(${sx}, ${sy})`;
    const terug = tussen(t, L.draai + L.om, L.draai + L.om + L.terug);
    if (om >= 1) laag.parentNode.classList.remove('intro-wacht');
    woord.style.transform = om < 1 ? '' : `perspective(${persp}px) rotateY(${(1 - uitTerug(terug, 1.4)) * -90}deg)`;
    // 3. de ster springt in een boog naar zijn plek, en draait een keer rond
    const vl = inUit(tussen(t, L.draai + 30, land));
    const p0 = [mix(rMidden[0], ster0[0], plof), mix(rMidden[1], ster0[1], plof)];
    const x = mix(p0[0], ster1[0], vl), y = mix(p0[1], ster1[1], vl) - Math.sin(vl * Math.PI) * L.boog * W;
    const druk = Math.sin(tussen(t, land, land + 250) * Math.PI);
    const bult = sterDoel * (1 + .35 * Math.sin(vl * Math.PI));
    ster.style.opacity = t < land + 100 ? 1 : 1 - tussen(t, land + 100, land + 250);
    ster.style.transform = `translate(${x}px, ${y}px) rotate(${vl * -366}deg) ` +
      `scale(${mix(sx * plof, bult, vl) * (1 + .15 * druk)}, ${mix(sy * plof, bult, vl) * (1 - .15 * druk)})`;
    if (!geland && t >= land) { geland = true; logoIntroVonken(laag, ster1); }
    // 4. het woord veegt achter de ster aan open, met een zachte rand
    const rand = t < land ? Math.max(L.rRand * k, x - 12)
      : mix(ster1[0] - 12, W * 1.05, 1 - Math.pow(1 - tussen(t, land, land + L.open), 3));
    const pr = rand / W * 100;
    woord.style.webkitMaskImage = woord.style.maskImage = pr >= 100 ? '' : `linear-gradient(90deg, #000 ${pr}%, transparent ${pr + 4}%)`;
    zetRaf(requestAnimationFrame(beeld));
  }
  // Het eerste beeld meteen, niet pas bij het volgende frame: anders staan de
  // twee tekeningen één frame lang onbewerkt op het scherm, groot en los.
  beeld(t0);
}
...
function goMap(vanLvl) {
  ...
  const o = kaartOpdracht(p, vanLvl, uitShow);
  const travel = o.travel, gekozen = o.gekozen, thuis = o.thuis, onthul = o.onthul;
  ...
  /* Pas hierna, en niet in een requestAnimationFrame: renderTourMap kon zijn
     ...
     wel ná kaartVertrek hierboven staan: die leest kaartFocus om te zien of de
     zaal moet wijken, en kaartTerugZoom (via tourMapVoltooi) verbruikt diezelfde
     kaartFocus. Verwissel die twee regels en de terug-grendel wordt gezet op een
     vlag die al leeg is. Zie de regel bij tourMapVoltooi en bij renderTourMap. */
...
/* Wat moet déze kaart doen?

   Vier opdrachten liggen klaar als briefje, en dit is de enige plek die ze
   opmaakt: gelezen, meteen op null, en verder werken met de kopie. Wat eruit
   komt zegt precies één ding -- welke wereld er in beeld komt en wat er daarna
   nog moet gebeuren -- en verandert verder niets aan het scherm.

   Stond hier eerder als zeventig regels midden in goMap. Ze horen bij elkaar
   (alle vier gaan ze over dezelfde vraag) en ze horen niet bij de rest (die
   gaat over tekenen en bewegen), dus staan ze nu apart. Er is geen regel aan
   veranderd; wel is nu in één oogopslag te zien dat de vier elkaar uitsluiten.

   netAf wordt hier soms gewist en niet gezet -- goMap zet hem vlak hiervoor, en
   alleen de reis-binnen-een-wereld neemt hem weer weg. Zie de uitleg daar. */
function kaartOpdracht(p, vanLvl, uitShow) {
  ...
  return { travel: travel, gekozen: gekozen, thuis: thuis, onthul: onthul };
}
/* Het scherm waar je vandaan komt, en hoe het vertrekt.

   Twee takken die elkaar uitsluiten: terug uit een show (de zaal wijkt, en de
   terug-grendel gaat dicht) of een gewone wissel (de kaart komt ondoorzichtig
   op en het vorige scherm dooft). Een reis heeft geen van beide nodig --
   reisNaarWereld beweegt de kaart zelf.

   Leest kaartFocus en niet uitShow: goMap zet die twee vlak na elkaar, maar het
   is kaartFocus die zegt of er een halte is om naartoe te zoomen. */
function kaartVertrek(vorige, gekozen) {
...
function runWorldReveal(o) {
  ...
  const vier = () => {
    sndWereld();   // een wereld die opengaat, niet de kassa -- zie de SFX-tabel
    confetti(26);
...
function runTravel(t) {
  ...
  const vier = () => {
    ...
    /* HET TIKJE VAN AANKOMEN ZIT IN sndAankomst -- tril 15, zie de SFX-tabel. Niet
       ...
       En het zijn er twee geworden, want vier() bedient twee momenten. Binnen een
       wereld kom je aan op de volgende halte; over een grens heen gaat er een héle
       wereld open, en dat is waar de camera net overheen geklommen is. Hetzelfde
       onderscheid dat de confetti en de juichtekst hieronder al maakten -- dit
       leest dus dezelfde vlag en verzint er geen tweede. */
    if (crossing) sndWereld(); else sndAankomst();
  ...
  setTimeout(() => {
    /* Hier zet ze af, en dit is de enige plek waar dat een tijdstip is: REIS_WACHT
       is exact de vertraging van de loopanimatie hierboven, dus er komt voor het
       vertrekklankje geen tweede klok bij. Dezelfde voorwaarde als de vonkjes --
       staat ze er niet meer (weggenavigeerd tijdens de wachttijd), dan vertrekt er
       ook niets. Bij beperkte beweging komen we hier niet eens: die tak gaat
       hierboven al rechtstreeks naar finish(), en dan is er geen wandeling om aan
       te kondigen. */
    if (hero.isConnected) sndVertrek();
...
function wereldFeest(f) {
  ...
  if (aan) aan.onclick = (e) => {
    ...
    sndAan();              // dezelfde handeling als in de kleedkamer, dus hetzelfde klankje
    const q = P();
...
function surpriseOutfit() {
  ...
  /* Dit was de enige gebeurtenis in de hele app die wél trilde en niet klonk: een
     buzz(20) zonder noot. Nu hetzelfde klankje als elk ander aandoen -- en de tril
     van 20 zit erin, dus hij is niet weg maar verhuisd. */
  sndAan();
}
...
function equipShopItem(id) {
  ...
  herbouw ? renderShop() : paintShop();
  // Een stil danspasje, geen tweede geluidje: sndAan() hierboven heeft het aandoen
  // al gezegd, en wat er te zien valt is dat zíj iets anders aanheeft.
  dance('shop-avatar');
}
